...
export function DashboardBanner({ stats, isLoading }: Props) {
    if (isLoading || !stats) {
        return (
            <div className="bg-forest-deep text-white p-4 mx-4 mt-4 rounded-xl shadow-lg gold-border animate-pulse h-32">
                ...
            </div>
        );
    }
    ...
    return (
        <div className="bg-forest-deep text-white p-5 mx-4 mt-4 rounded-xl shadow-lg gold-border relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gold-border opacity-10 rounded-full blur-2xl -mt-10 -mr-10"></div>

            <h2 className="text-xl font-bangers mb-4 flex items-center gap-2 metallic-text">
            ...
            {!hasBets ? (
                <div className="text-center text-cream/40 text-sm py-4 italic font-fredoka">
                    Aún no has apostado en ninguna partida.
                </div>
            ) : (
                <>
                    <div className="grid grid-cols-2 gap-2 text-center mb-3">
                        <div className="bg-forest-mid/50 rounded-lg p-2 border border-gold-border/20">
                            <div className="text-[10px] text-cream/50 uppercase tracking-wider mb-1 font-bangers">Total Apostado</div>
                            <div className="text-sm font-fredoka font-semibold text-cream">{formatCurrency(stats.wagered)}</div>
                        </div>
                        <div className="bg-forest-mid/50 rounded-lg p-2 border border-gold-border/20">
                            <div className="text-[10px] text-cream/50 uppercase tracking-wider mb-1 font-bangers">
                                {stats.realizedNet !== 0 ? 'Ganancia / Pérdida' : 'Apuestas'}
                            </div>
                            <div className={`text-sm font-fredoka font-semibold ${stats.realizedNet > 0 ? 'text-green-400' : stats.realizedNet < 0 ? 'text-team-red' : 'text-cream'}`}>
                                {stats.realizedNet !== 0
                    ...
                    {stats.closedWagered > 0 && (
                        <div className="mt-3 text-xs">
                            <div className="flex justify-between text-cream/40 mb-1 font-fredoka">
                                <span>Recuperado</span>
                                <span>{Math.round((stats.closedRecovered / Math.max(1, stats.closedWagered)) * 100)}%</span>
                            </div>
                            <div className="h-1.5 w-full bg-forest-mid rounded-full overflow-hidden">
                                <div
